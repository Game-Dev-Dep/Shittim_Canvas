...
public class Win32Wrapper
{
    ...
    public static IntPtr FindUnityWindow(string windowTitlePart = "Shittim Canvas")
    {
        ...
        EnumWindowsProc callback = (hwnd, lParam) =>
        {
            ...
            if (windowProcessId == currentProcessId && IsWindowVisible(hwnd))
            {
                ...
                // 检查标题是否包含关键词（默认"Unity"）
                if (windowTitle.ToString().Contains(windowTitlePart))
                {
                    foundWindow = hwnd;
